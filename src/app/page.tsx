"use client";
import { Box, Container, Typography, Button, useTheme, Dialog, IconButton } from '@mui/material';
              ...
              <Box key={car.name} className="glass-card flex flex-col overflow-hidden group">
                ...
                <Box sx={{ p: 4, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', bgcolor: 'background.paper', borderTop: '1px solid', borderColor: 'divider' }}>
                  ...
                  <Button 
                    ...
                    fullWidth 
                    size="large"
                    onClick={() => handleOpenBooking(car.name === 'Baleno' ? 'Baleno MT' : `${car.name} ${car.type}`)}
                    sx={{ fontWeight: 'bold', py: 1.5, boxShadow: '0 4px 14px 0 rgba(212, 175, 55, 0.39)', '&:hover': { boxShadow: '0 6px 20px rgba(212, 175, 55, 0.23)' } }}
                  >
                    Book Now
                  </Button>
                </Box>
